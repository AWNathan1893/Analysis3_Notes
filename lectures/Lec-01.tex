...
We will talk about $ n $-variable calculus, that is, calculus on $ \R^n $. Recall the following:
\begin{enumerate}[label = $\bullet$]
  ...
  \item $ \R^n $ is an $ n $-dimensional inner product space over $ \R $, with the standard orthonormal basis $ \{e_j\}_{j=1}^n $
\end{enumerate}
...
\section{Review: \texorpdfstring{$ \R^n $}{Rn} as a vector space\index{Vector Space}}
...
\begin{enumerate}[label = (\roman*)]
  ...
  \item Linear maps\index{Linear map}
...
We have the Euclidean norm\index{Euclidean norm} on $ \R^n $ defined by,
\[ \norm{x} = \left(\sum_{i=1}^{n}x_i^2\right)^{\frac 12} \quad \forall \, x \in \R^n \]
This induces the metric\index{Euclidean metric} given as,
\[ d(x,y) = \norm{x-y} = \left(\sum_{i=1}^{n}(x_i-y_i)^2\right)^{\frac 12} \quad \forall \, x,y \in \R^n \]

\begin{Thm}{Cauchy-Schwarz Inequality}{}\index{Cauchy-Schwarz Inequality}
...
\begin{Thm}{Triangle inequality for $ \R^n $}{}\index{Triangle inequality}
  For all $ x,y \in \R^n $,
  \[ \norm{x+y} \leq \norm{x} + \norm{y} \]
\end{Thm}
\begin{proof}
...
The following is a technical result, which can be thought of as an analogue of the Lipschitz\index{Lipschitz} condition for linear maps on $ \R^n $. It hints towards continuity of linear maps, and we will see that it is indeed so later.
\ssk
